...
Lors du précédent chapitre, vous avez révisé et découvert plusieurs sortilèges de la famille `ggformula`. Parmi ces derniers, `gf_point` est 
un sortilège permettant de représenter des nuages de points. Souvenez-vous : il faut introduire des ingrédients dans la recette du sortilège : 
en l'occurrence, l'ordonnée y des points en fonction de leur abcisse x. Cela se traduit en langage des Runes par&nbsp;: `gf_point(y~x)`.
...
utiliser le signe `|` (un trait vertical).
...
modalité de la variable "situation". 
