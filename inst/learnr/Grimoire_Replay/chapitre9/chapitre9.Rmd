...
Ce registre est déjà chargé, dans un data.frame `naissances`. Nous savons découvrir le contenu 
d'une table avec les instructions `names()` pour avoir le noms des variables, `head()` pour consulter les premiers enregistrements, et `nrow()`
...
><details><summary><font size="3"><b>Pour les plus téméraires 3 : partie optionnelle sur la moyenne pondérée et la médiane pondérée - cliquez ici</b></font></summary>
><p><font size="2">
>
><BR>
>
...
De même que ci-dessus, on ajoute `group_by()` :  
- au sortilège `summarise()`,  
