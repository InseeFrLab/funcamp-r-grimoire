...
Rappel : on peut compter le nombre de lignes du data.frame avec le sortilège "nrow".
```{r}
...
- le sortilège `table()`,  
- et ensuite le sortilège `addmargin(FUN = list(Total = sum))`.  
...
Nous vous proposons de recalculer ci-dessous des nombres de cuvées. Au fait, dans la console R, vous pouvez également afficher la solution pour avoir une aide avec le bouton :
...
# suivant les valeurs des variables qualitatives "Exploitation" et "Temps",
# avec le sortilège "table()",
# avec ajout du nom des variables qualitatives, au tableau de résultats
...
Pour ajouter les totaux, on applique ensuite le sortilège `addmargin(FUN = list(Total = sum))`, comme ci-dessus.
...
table(df_livre$Exploitation, df_livre$Temps, deparse.level = 2) %>%
  prop.table(margin = 1) %>%
  addmargins(FUN = list(Total = sum), margin = 2)
```
...
table(df_livre$Exploitation, df_livre$Temps, deparse.level = 2) %>%
  prop.table(margin = 2) %>%
  addmargins(FUN = list(Total = sum), margin = 1)
...
On peut passer à des sommes en pourcentage, en pourcentage en ligne ou en pourcentage en colonne, comme ci-dessus, avec 
le sortilège `prop.table`.
...
# Calculer la somme de la variable "Production" du data.frame "df_livre",
# suivant les valeurs des variables qualitatives "Exploitation" et "Temps",
...
### Moyennes d'une variable quantitative d'un `data.frame`, suivant les valeurs de plusieurs variables qualitatives, et présentation dans un tableau
...
><BR>
>
>Dans cette sous-partie, on reprend le dernier morceau de code.
>
><BR>
>
>Puis on utilise le sortilège "xtabs()" pour présenter les résultats dans un tableau avec une variable en ligne et une variable en colonne.
>
><BR>
>
>```{r, message=FALSE, warning=FALSE}
...
# Puis utiliser les sortilèges "summarise()", "mean()", et nommer la moyenne "moyenne_production".
...
# suivant les valeurs de la variable qualitative "Exploitation".
# Pour cela, je pars du data.frame "df_livre", puis j'utilise les sortilèges "group_by", "summarise", et "sd" (standard deviation)
