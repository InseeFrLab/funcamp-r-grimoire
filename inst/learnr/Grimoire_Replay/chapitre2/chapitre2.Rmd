...
Aidons IcaRius à retrouver ce précieux document. Il n'est pas tout à fait perdu : il en reste une copie dans un fichier. 
Mais pour corser le tout, le fichier semble indéchiffrable... Il est en effet dans un format ".RData".

**Aie aie aie.**

<BR>

Comment lire le contenu d'un fichier dans R ?

<BR>

<BR>

## Les fichiers usuels au Royaume de Statis
...
Le sortilège `import()` permet d'importer dans R les fichiers qui sont sur disque dur (ou sur clé USB, etc.) 
et qui ont une extension ".sas7bdat" (SAS), ".xls", ".xlsx", ".ods" (Libre office Calc), ".csv", 
...
#### Charger un fichier ".RData" dans R
...
Un fichier "livre_compte_3.RData" existe.  
...
# (BDM, Banque de Données Macro-économiques), au format sdmx.
# Je souhaite charger ces chiffres dans R.

# J'importe les données, avec le sortilège "readSDMX",
...
Nous vous proposons de charger les données "https://bdm.insee.fr/series/sdmx/data/SERIES_BDM/000868205" dans R.
...
# Pour faciliter l'utilisation, transformer "naissances_93" en un "data.frame", 
...
>S'il y a une difficulté d'encodage dans les lignes ci-dessus (par ex. si des lignes "Naissances domiciliÃ©es par rÃ©gion" apparaissent),
...
><BR>
>
>Dans cette sous-partie, on demande à R de lire avec l'encodage "UTF-8" et d'afficher, la série téléchargée.  
...
# Transformation en un data.frame "df_naissances_93"
df_naissances_93 <- as.data.frame(naissances_93)

####
## data.frame "df_naissances_93"
# je modifie les colonnes de type "facteur" en colonnes de type caractère
...
# je déclare que l'encoding de chaque colonne est "UTF-8"
for (name in colnames(df_naissances_93)){ # emploi de "for" pour énumérer, et de "colnames()" pour obtenir les colonnes d'un data.frame
    Encoding( df_naissances_93[,name] ) <- "UTF-8" # utilisation du sortilège "Encoding()"
}
####

# Lecture et affichage (la lecture de la table a alors lieu en UTF-8)
head(df_naissances_93, 5)
>```
>
>
...
> et de demander à R de les lire avec l'encodage UTF-8 et de les afficher .
>
>```{r ok-chapitre2-etape3,exercise=TRUE, exercise.setup = "prepare-data", message=FALSE, warning=FALSE}
# Télécharger les données et les enregistrer dans un objet R "naissances_93"


# Pour faciliter l'utilisation, transformer "naissances_93" en un data.frame "df_naissances_93"


####
## data.frame "df_naissances_93"
# modifier les colonnes de type "facteur" en colonnes de type caractère


# déclarer que l'encoding de chaque colonne est "UTF-8"


####

# Lire et afficher les 5 premières lignes du data.frame
...
# modifier les colonnes de type "facteur" en colonnes de type caractère
df_naissances_93 <- df_naissances_93 %>%
    mutate_if(is.factor, as.character) # utilisation de "mutate_if()", "is.factor()", "as.character()"
...
#### Demander à R de télécharger un fichier disponible sur internet

<BR>

Avec le sortilège `download.file()`, on peut demander à R de télécharger un fichier sur internet.
...
Nous vous proposons de demander à R :  
...
# Je souhaite que le résultat du dé-zippage apparaisse dans un dossier "monRepertoire".
...
A ce stade, un petit rappel, si vous le souhaitez.
...
Souhaitez vous réviser le "pipe" (revoir comment le "pipe" s'emploie, comment il permet d'assembler les sortilèges) ?
...
# Table "df_statis". En utilisant le "pipe" %>% et le sortilège print(), 
