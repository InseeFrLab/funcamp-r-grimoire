...
Outre le fait de vous guider dans le labyrinthe, savoir utiliser un vecteur vous sera d'une aide précieuse 
pour des opérations portant sur les tables de données, comme la sélection d’observations et de colonnes.
...
# J'obtiens les éléments du vecteur "nombre_d_equipements" supérieurs ou égaux à 2
nombre_d_equipements[ nombre_d_equipements >= 2 ]
```
Il y a trois éléments du vecteur "nombre_d_equipements supérieurs ou égaux à 2
...
On écrit "ou" avec le signe `|` (trait vertical)
```{r, messages=FALSE, warning=FALSE}
...
```
Deux éléments du vecteur sont inférieurs ou égaux à 4 et supérieurs ou égaux à 2.
...
#### On peut parfois indiquer à R d'enlever les valeurs manquantes dans les opérations
...
# Le data.frame "data.frame_clef" est déjà chargé dans R.
# Accéder à la colonne "col_1" du data.frame "data.frame_clef" avec le signe $


# Tenter de calculer la moyenne des valeurs du vecteur data.frame_clef$col_1,
# avec le sortilège "mean()",
# sans utiliser d'ingrédient particulier


# Ré-essayer d'obtenir la moyenne des valeurs du vecteur data.frame_clef$col_1,
# avec le sortilège "mean()", 
