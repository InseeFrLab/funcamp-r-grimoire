...
Ce registre est déjà chargé, sous le nom de `naissances`. Souvenez-vous nous avons appris à découvrir le contenu 
...
Nous allons désormais apprendre à utiliser des sorts de l'extension `dplyr` pour calculer à partir d'une table, et
des fonctions de `tidyr` pour transposer un data.frame. 
...
Nous allons désormais utiliser les sortilèges apportés par `dplyr` et `tidyr` pour étudier le registre des naissances.
...
On considère le data.frame "naissances" (ou un autre data.frame avec des observations en ligne, et des variables en colonne).
...
# Obtenir le nombre d'observtions du data.frame "naissances"
...
### Ajout du sortilège `group_by` pour mener les opérations pour chaque modalité d'une ou de plusieurs variables qualitatives
...
><details><summary><font size="3"><b>Pour les plus téméraires : obtenir les résultats dans un tableau - cliquez ici</b></font></summary>
><p><font size="2">
>
><BR>
>
...
# Je calcule le nombre d'observtions du data.frame "naissances",
# suivant les modalités des variables "Village" et "An".
...
- `summarise()`,  
- et à l'une des fonctions `median()`, `sum()`, `quantiel()`, etc. 
