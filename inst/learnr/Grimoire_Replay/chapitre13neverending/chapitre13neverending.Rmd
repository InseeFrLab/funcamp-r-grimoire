...
Soyez les bienvenus : vous voici dans l'environnement de développement RStudio. Prenez le temps de trouver vos marques. L'environnement est prêt : à gauche le panneau source, à droite le viewer. Sentez-vous libre de manipuler les fenêtres comme bon vous semble. 
...
Juste avant de finaliser l'aventure, nous vous proposons un encadré optionnel,
 pour créer un "projet de package" comme vu dans la première partie du chapitre 13.
 
Si vous souhaitez créer un "projet de package :   
1. Enregistrez l'encadré ci-dessous dans un fichier,  
...
Choisir "New Project..."  
Si une fenêtre pop up "Save workspace image to ~/.RData?" apparaît, cliquer sur "Don't save".
...
# Le projet est créé. Il contient déjà un sortilège "hello()" ainsi qu'une documentation 
# de ce sortilège.  
# Dans RStudio ouvrir une fenêtre de script, puis exécuter le code suivant :  
library(devtools)  
load_all() # chargement en mémoire des sortilèges du projet (ici hello() )
hello() # exécution
