...
Dans cette sous-partie on va :  

- considérer un fichier qui contient du code R,  

- ajouter des commentaires à ce fichier,  

- créer une fiche descriptive DESCRIPTION,  

- et utiliser des sortilèges magiques qui vont générer une documentation automatiquement.  
...
fmt <- function(x, n) { # "fmt" prend en entrée deux paramètres : la quantité à formater, et le nombre de décimales
  ...
  format(x,              # "fmt" utilise "format()" du R de base, qui permet de formater
         decimal.mark = ",",  		 # la virgule est employée pour les nombres décimaux
         digits=0, nsmall=n, 		 # on utilise l'écriture "digits=0, nsmall=n", pour n décimales
...
#' le nombre de decimales specifié, 
#' la virgule pour distinguer unités et décimales, 
#' l'espace comme séparateur de milliers. 
#' @param x la quantité à formater
#' @param decimales le nombre de decimales
#' @import dplyr
#' @examples
#' fmt(100, 2)
#' fmt(1234.56789, 2)
fmt <- function(x, nombre_decimales) {

  format(x,                    
         decimal.mark = ",",  
         digits=0, nsmall=nombre_decimales,
         big.mark = " ",      
         scientific = FALSE   
  )      
...
*Les commentaires "roxygen2" peuvent s'employer en plus des commentaires traditionnels commençant par #*
...
À vous de documenter ces sortilèges. Dans la fenêtre ci-dessous, vous pouvez écrire librement, sans crainte aucune de commettre une erreur (rien ne sera exécuté par R).
...
- mais aussi de fabriquer automatiquement des pages de documentation, comme celles qu'on peut afficher avec le lien suivant :
```{r, echo=FALSE}
library(shiny)
library(htmltools)
...
Pour générer les pages ci-dessus automatiquement, on peut utiliser le code suivant :
...
*- pour ajouter des hyperliens, la syntaxe est particulière.*  
*`\url{...)` va ajouter un lien internet*  
*`\code{\link{...}}` ajoute un lien vers une autre fonction (du même projet RStudio)*  
...
À vous de documenter les sortilèges. Dans la fenêtre ci-dessous, vous pouvez écrire librement, sans crainte aucune de commettre une erreur (rien ne sera exécuté par R).
...
#' utilisent la virgule au lieu du point pour les nombres decimaux, et l'espace " " pour separer les milliers.
#'
#' @docType package
