...
# création du répertoire sauvegarde
...
## fichier "chapitre10_replay/naissances_replay.csv" : même fichier que le fichier "naissances_replay.csv" du chapitre9_replay
## fichier "chapitre10_replay/data_soldiers.csv" :     même fichier que le fichier "data_soldiers.csv"     du chapitre6_replay

# Import des tables pour l'exercice d'export
file.copy(from = system.file(package = "funcampR","data","chapitre10_replay/naissances_replay.csv"),
          to = paste0(tempdir(),"/naissances_replay.csv"))
...
naissances<-import(paste0(tempdir(),"/naissances_replay.csv"))
# naissances_F
naissances_F <- naissances %>% 
  filter(Genre=="F")

# chemin_naissances
chemin_naissances <- paste0(
  tempdir(),
  "/naissances.rds"
)
chemin_naissances <- str_replace_all(chemin_naissances, "\\\\", "/")
repertoire_naissances <- tempdir()
repertoire_naissances <- str_replace_all(repertoire_naissances, "\\\\", "/")
...
donnees_secretes <- donnees_secretes %>%
  select('royaume','nb_guerriers','nb_habitants','nb_chevaux','nb_canicule','dist_royaume','situation', 'vegetation')
...
# Création du data.frame df_livre_ambroisie
file.copy(
  from = system.file(
    package = "funcampR",
	"data",
...
# suppression du fichier "tempdir()/livre_ambroisie.fst"
tryCatch(
  {
...
# donnees_secretes, graphique "graphique_chapitre6"
donnees_secretes <- import(paste0(tempdir(),"/data_soldiers.csv"))
donnees_secretes <- donnees_secretes %>%
...
```{r prepare-data_2}
# suppression "tempdir()/naissances.rds"
tryCatch(
  {
    if (file.exists(paste0(tempdir(),"/naissances.rds"))) {
      unlink(paste0(tempdir(),"/naissances.rds"))
    }
  },
  error=function(cond) {
    return("")
...
```{r prepare-data_3}
# suppression "tempdir()/livre_ambroisie.fst"
tryCatch(
  {
...
```{r prepare-data_4}
filename_livre_ambroisie <- paste0(
  tempdir(),
  "/livre_ambroisie.fst"
)

# création "tempdir()/livre_ambroisie.fst"
tryCatch(
  {
    if ( !file.exists(paste0(tempdir(), "/livre_ambroisie.fst")) ) {
      write_fst(
...
## Enregistrer dans des formats de fichiers du langage des Runes
...
save.image("chemin complet du fichier .Rdata")
```
...
- le chemin du fichier RData dans lequel va être stocké l'objet.  
```{r, collapse = TRUE, eval=FALSE}
save(nom_objet_à_sauvegarder,file="chemin complet du fichier .RData")
```
...
```{r, collapse = TRUE, eval=FALSE}
save(table1_à_sauvegarder,vecteur_à_sauvegarder,table2_à_sauvegarder,file="chemin complet du fichier .RData")
```

**Prêt pour l'échauffement ?** Pour parfaire votre apprentissage du sortilège `save`, exercez-vous en sauvegardant dans un même "RData" nommé "tables_chapitre9", 
les 2 tables suivantes: "naissances" et "naissances_F", du registre des naissances rencontré au chapitre 9 "Do you dplyr?".
...
# Sauvegarder les tables "naissances" et "naissances_F"
...
# Sauvegarder les tables "naissances" et "naissances_F"
save(naissances,naissances_F,file="../sauvegarde/tables_chapitre9.RData")
# Fin de l'exercice
``` 
...
Un fichier RData pourra ensuite être ouvert et lu à tout moment grâce au sortilège `load("chemin du fichier")`, vu dans le chapitre 2.

A la suite de `load("chemin du fichier")`, tous les objets sauvegardés dans le fichier seront chargés dans R.
...
str_1 <- 
  paste0(
    "# Je sauvegarde le data.frame \"naissances\" dans un fichier \".rds\",\n",
	"# dans le répertoire \"",
	repertoire_naissances,
	"\"\n",
...
str_2 <- 
  paste0(
    ...
    "library(dplyr)\n",
	"\n",
    "# J'importe dans R le fichier \"naissances.rds\".\n",
...
Nous vous proposons d'enregistrer la table "naissances" dans un fichier "naissances.rds", dans le répertoire &nbsp;&nbsp; `r repertoire_naissances` .  
...
# Avec le sortilège "saveRDS", enregistrer la table "naissances" 
# dans le fichier "naissances.rds"


# Importer le fichier "naissances.rds" dans R, avec le sortilège "readRDS".
...
#### Le format ".fst" pour enregistrer un très gros data.frame

<BR>

Le format ".fst" permet de sauvegarder un data.frame dans un fichier.

Ce format est intéressant pour l'enregistrement d'un très gros data.frame (de typiquement 1 Go, ou plus).
...
- la puissance de la liqueur, comparable au nombre de degrés d’une boisson alcoolisée,  
...
fichier "livre_ambroisie.fst", dans le repertoire `r repertoire_livre_ambroisie` .  
(le système a vérifié que le répertoire existe).
...
str_6 <-
  paste0(
    "# Créer une référence au fichier avec le sortilège \"fst()\" \n",
    "ref_livre_ambroisie <- fst(filename_livre_ambroisie)\n",
    "\n",
...
## Et si je veux sauvegarder des données dans des formats usuels (txt, jpg, csv, etc.) ?
...
export(nb_pontes, "../sauvegarde/nb_pontes.csv")
```
...
À vous de jouer : vous devez désormais exporter la table naissances au format csv dans un fichier nommé naissances.csv. 
...
```{r ok-chapitre10-etape5,exercise=TRUE, exercise.setup = "prepare-data", message=FALSE, warning=FALSE}
# Sauvegarder la table "naissances" au format csv
...
# Sauvegarder la table "naissances" au format csv
export(naissances,"../sauvegarde/naissances.csv")
# Fin de l'exercice
``` 
...
```{r ok-chapitre10-etape6,exercise=TRUE, exercise.setup = "prepare-data", message=FALSE, warning=FALSE}
