...
- on crée une fiche signalétique,  
...
Dans toute la suite, on dispose d'un fichier `fmt.R` (une page du grimoire d'Icarius).
...
*Chaque sortilège est précédé par un bloc de commentaires "roxygen2".*  
...
(La fiche signalétique est requise pour les "projets de package".  
...
*(N.B. Pour générer complètement la documentation, il y a besoin aussi d'une étape d'installation*  
...
shinyApp(ui, server,
  options = list(height = 100) # espace réservé dans le document html produit
)
```
...
Dans cette partie, on adjoint d'autres commentaires au script `fmt.R` ci-dessus.

<BR>

On utilise des mots-clés suppplémentaires proposés par le package **roxygen2** :
```{r, messages=FALSE, warning=FALSE}
#' @title fmt
...
*- si l'on souhaite ajouter des hyperliens dans la documentation qui sera générée, la syntaxe est particulière :*  
&nbsp;&nbsp;*`\email{...}` ajoutera un lien vers des outils de messagerie,*  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*(il faut écrire `@@` dans l'adresse email),*  
&nbsp;&nbsp;*`\url{...)` insèrera un lien internet,*  
&nbsp;&nbsp;*`\code{\link{...}}` faira apparaître un lien vers une autre fonction (du même projet)*  
...
Vous avez terminé ? Votre script commenté devrait ressembler à ce qui figure ci-dessous. 
