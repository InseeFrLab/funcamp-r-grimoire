...
The `names()` function is used to find out the different variables included in a file, or in other words to show columns' names in the data table:
...
We now have the magical power to load files of different formats into R, and to know their variables. The few examples here are far from being exhaustive: with `import()`, you could also load files in SPSS, Stata, Matlab ... and of course, R data files. We will find out later in the grimoire how R saves objects you have created. But for now, let's get back to the egg-laying book.

> <details> <summary> <font size = "3"> <b> For the dauntless: optional part on CSV files - click here </b> </font> </summary>
> <p> <font size = "2">
>
> The egg laying book for the 4th week comes from a fragment that ends with the `.csv` extension. So what is a CSV file?
>
> CSV is a file format where each record appears on its own line and whose fields are separated by a single character called a “separator”. In this case, the separator is a comma or a semicolon: this is the meaning of the acronym CSV, for Comma Separated Value.
>
