...
- Enregistrez la succession d'étapes ci-dessous pour créer un projet de package, par exemple sur un fichier personnel, car la création d'un projet de package
risque de rendre ces lignes inaccessibles.  
- Tentez la création d'un projet de package que vous appellerez "projet1" ou d'un autre nom si vous le souhaitez.  
- puis vous pourrez reprendre le jeu vidéo icaRius et donner la réponse (le code magique ci-dessus) à Asa la gueRnouille.

###############################################
###############################################
Création d'un projet de package, succession d'étapes :  
###############################################
Dans RStudio, tout en haut à droite, cliquez sur "chapitre 13".
Une liste déroulante apparaît. Choisir "Close project"
cliquer sur "Project: (None)".  
...
# Le projet est créé. Il contient déjà un sortilège "hello()" ainsi qu'une documentation de ce sortilège "hello()"
# Ouvrir une fenêtre de script, puis exécuter le code suivant :
library(devtools)
load_all()
hello() # exécution du sortilège "hello()"
?hello # affichage de la documentation du sortilège "hello()"
###############################################
