...
- understand what is the famous runic R language
- start the first dialogues with R using an input window: the console
- familiarize ourselves with the tics of R language

If necessary, do not hesitate to request one of the quizmasters moving around the room.

But let's start… at the beginning! R is a language used for processing statistical data, the first version of which was published in 2000. First disseminated among researchers community, R is now used more and more within national statistical institutes. 

R allows you to make **statistical computations**, to present them for instance as **tables** and also to draw **graphs** and geographical maps. The results produced with R can be exported into **reports** and can even be integrated in **web applications**. Mastering the Language of the Runes is then a great power ~that comes with great responsibility~!
...
In the adventure of icaRius, you will sometimes be asked to answer quizzes to help the hero continue his quest. Here is a very first one, just to familiarize yourself with the format of the exercise. So it's up to you to respond directly by checking your choice and then pressing the button called `Submit answer`.
...
Here is a slightly larger console that has some pre-written instructions, always run them with the button to see how R structures its response to your instructions
...
The console is a place of dialogue : you command and R does it! Note that you see all the results appear one after the other (for example, there are 3 calculations requested here separately, and R gives you back 3 results): therefore, there is **one answer per instruction**.
...
In R, information can be stored in what we will call **objects**: to illustrate, an object is like a box on which we can write a label (a name) and in which we store a content (the result of a calculation for example). Giving names to objects helps you navigate long calculations.

For example, we are going to create an object named `n_lives`, that will be used to store the number of the hero's lives in the game. Icarius starts its journey with 3 lives, therefore the value we want to give to `n_lives` is 3. To store the value 3 in the object called `n_lives`, the runic language has a very particular sign, namely the arrow `<-`. It is constructed by concatenating the sign `<` with the dash `-`. The assignment of a value to an object is performed with the arrow `<-`. For example:
...
In the world of Statis, age follows a somewhat strange logic. At birth, the age of the statisian corresponds to their number of lives multiplied by pi squared, from which we subtract the maximum number of lives that they will be able to have during their journey… To facilitate the whole, one will admit to take the rounding of the result...
