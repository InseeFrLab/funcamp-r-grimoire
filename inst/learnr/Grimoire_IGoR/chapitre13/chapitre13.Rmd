...
Depuis vos mésaventures avec Teoc vous savez maintenant écrire votre propre sortilège et le documenter consciensieusement, et votre mémoire est encore fraîche. Vous décidez de vous équiper non plus d'un livre mais d'une tablette magique, bien plus moderne et surtout, à toute épreuve pour consigner vos savoirs-faire.
...
Quelle est donc cette magie noire ?! Encore un coup de Sassos ? Vous qui vous attendiez à simplement scripter vos sortilèges avec votre plume du tidyverse vous voici bien dépourvu...
...
Vu l'âge avancé des mages, vous n'êtes pas étonnés qu'il doivent être assisté pour la rédaction de leur sortilèges... Mais voyons plutôt comment. 
...
A la droite de cet encart, on trouve un panneau qui contient un explorateur de fichier et d'autres onglets utiles tels que l'affichage des graphiques, la liste des packages installés, la fenêtre d'aide et même un navigateur web (Viewer). 
...
Et enfin, un dernier quart de page est dédié à l'écriture à proprement parlé des sortilèges : 
```{r  echo=FALSE}
...
La dimension "intégrée" de l'outil prend son sens en ce que tout est à portée de main pour parler le langage des Runes. Notez que tout cet environnement est customisable à l'envi et vous pouvez donc vous saisir des "poignées" pour réajuster la taille des fenêtres comme bon vous semble.
...
Un document RMarkdown est donc un entrelacs de lieu où taper du texte et de lieux où taper du code. Ces derniers sont matérialisés par une zone grisée dans votre tablette RStudio : 

```{r  echo=FALSE}
include_graphics("images/premier_chunk.png")
```

Ces zones grisées d'exécution de R commencent par des symboles ``` (ce sont des accents graves ou backticks en anglais) et se terminent par cette même série de symboles. Il est préférables de ne pas toucher ces lignes là au risque de détraquer le document. Les lignes de code sont à taper entre ces deux lignes. 

L'intérêt de mélanger le texte et le code c'est qu'il nous est maintenant possible de produire des documents qui réalisent les calculs et les graphiques... et les mets à jour si les données changent ! Le rapport produit - on dit "compilé"- est alors est au format html, c'est-à-dire qu'il s'ouvre dans un navigateur web. Dans RStudio, on le trouvera dans l'onglet "Viewer" (visualiseur en anglais) : 
...
**Fin de la première partie du module 13  >>  lancez le service neverending qui contient RStudio **
